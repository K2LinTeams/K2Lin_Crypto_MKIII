...
export default function App(): React.ReactElement {
  ...
  useEffect(() => {
    async function loadTodos(): Promise<void> {
      const storedTodos = await window.api.store.get('todos')
      if (storedTodos) {
  ...
  const updateTodos = (newTodos: any[]): void => {
    setTodos(newTodos)
    window.api.store.set('todos', newTodos)
  }
      ...
      if (isEncrypted) {
        ...
        if (!encryptedObj) {
          ...
          if (currentFormat === 'Natural Text (Markov)') {
            const decodedBytes = await window.api.nlp.decode(outputData)
        ...
        const result = await window.api.crypto.decrypt(
        ...
        // Encrypt
        const result = await window.api.crypto.encrypt(inputData, secretKey)
  ...
  const formatOutput = async (data: any, format: Format): Promise<void> => {
    ...
    if (format === 'Base64') {
      ...
      const text = await window.api.nlp.encode(buffer.buffer)
      setOutputData(text)
    }
  }
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    if (isEncrypted && encryptedPackage) {
      ...
      try {
        ...
        const pngBuffer = await window.api.stego.embed(arrayBuffer, payloadBuffer.buffer)
      ...
      try {
        const payloadBuffer = await window.api.stego.extract(arrayBuffer)
